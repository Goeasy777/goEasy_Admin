...
function TermAndConditions() {
    ...
    async function fetchContent() {
      try {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/users/getTermsandConditions`
        );
  ...
  const handleSave = async () => {
    try {
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/users/addOrUpdateTermAndCondition`,
        { content }
      );
      if (response.data.success) {
        setShowAlert(true);
  ...
  return (
    <div className="relative bg-white p-5 h-full w-full rounded-md flex flex-col gap-3">
      ...
      <h2 className="ml-6 w-full text-left text-xl">Term & Conditions</h2>
