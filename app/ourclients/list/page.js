...
const page = () => {
    ...
    const [selectedOurClientId, setSelectedOurClientId] = useState(null);

    const fetchOurClient = async () => {
        try {
            const response = await fetch(
                `${process.env.NEXT_PUBLIC_API_BASE_URL}/users/getAllourHappyClients`
            );
            if (!response.ok) {
    ...
    const deleteHandler = async () => {
        try {
            const response = await fetch(
                `${process.env.NEXT_PUBLIC_API_BASE_URL}/users/deleteourHappyClients/${selectedOurClientId}`,
                { method: "GET" }
            );
            if (response.ok) {
                setShowModal(false);
                setData((prevData) =>
                    prevData.filter((client) => client._id !== selectedOurClientId)
                );
            } else {
    ...
    return (
        <div className="h-full w-full flex flex-col  py-10 px-14 gap-10 bg-formBG rounded-md">
            ...
            <div className="py-3 px-8 flex justify-between items-center text-xl w-full border-none shadow-sm rounded-sm bg-headerBG">
                <h2 className="text-textBlack">Our Clients List</h2>
                <LinkButton url="/ourclients/add" value="Add New Our Client" />
            </div>
            ...
            <div className="py-3 text-textBlack w-full grid grid-cols-[40px_1fr_1fr_1fr_1fr]  opacity-70 border-b-[1px] border-none">
                ...
                <h1 className="text-base text-center">Our Clients name</h1>
                <h1 className="text-base text-center">Our Clients Image</h1>
                    ...
                    <div>
                        {data.map((elem, index) => (
                            ...
                            >
                                ...
                                <h1 className="text-base text-center">{elem.clientName}</h1>
                                ...
                                {showModal && (
                                    <div className="fixed inset-0 flex items-center justify-center z-40">
                                        <div className="bg-white p-6 rounded shadow-sm w-96 z-50">
                                            ...
                                            <p>Are you sure you want to delete this our client?</p>
