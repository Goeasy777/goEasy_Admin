...
const page = () => {
  ...
  const addChildCategoryHandler = async (e) => {
    e.preventDefault();
    ...
    try {
      const response = await dispatch(addchildcategory(formData));
      ...
      if (response.success) {
        ...
        setShowAlert({
          type: "error",
          message: response.message || "Failed to add Childcategory.",
        });
      }
  ...
  const Status = () => {
    return (
      <div className="flex flex-col h-fit w-full gap-2">
        <h4 className="ml-3 text-lg  text-textBlack">Category Status</h4>
        <select
          className="p-3 shadow-sm border-none bg-headerBG"
          value={status}
  ...
  return (
    <div className="flex flex-col py-10 px-14 gap-10 rounded-md bg-formBG shadow-md">
      <h1 className="py-4 px-6 text-xl w-full border-none shadow-sm rounded-sm text-textBlack bg-headerBG">
        Add Child Category
      </h1>
      ...
      >
        ...
        <div className="grid grid-cols-2 w-full gap-3">
          ...
          <div className="grid grid-rows-2 gap-4">
            ...
            {/* Submit Button */}
            <Button props={"Add Child Category"} />
          </div>
