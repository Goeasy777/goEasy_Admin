  ...
  const getDisputes = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/users/getDispute?limit=${limit}&page=${page}`
      );
      if (!response.ok) {
  ...
  return (
    <div className="flex flex-col bg-formBG w-full h-full   py-10 px-14 gap-10 rounded-md ">
      ...
      <div className="py-3 w-full grid grid-cols-[40px_1fr_400px_1fr]  opacity-70 border-b-[1px] border-gray-100 text-textBlack">
        ...
        <h1 className="text-center text-base">User</h1>
        ...
        {data.length > 0 ? (
          <div>
            {data.map((data, index) => (
              <div
                key={data.id}
                className="py-3 w-full grid grid-cols-[40px_1fr_400px_1fr] items-center  border-b-[1px] border-gray-100 text-textBlack"
              >
