...
const page = () => {
  ...
  const fetchOrder = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/users/getcategoryorder?category=Pending&limit=${limit}&page=${page}`
      );
      if (!response.ok) {
  ...
  const SearchHandler = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/users/searchorder?title=${search}&category=Pending`
      );
      if (!response.ok) {
      ...
      <div>
        {search ? (
          searchResults.length ? (
              ...
              >
                ...
                <div className="flex items-center justify-center p-2 cursor-pointer">
                  <select
                    ...
                    className="bg-gray-50 rounded px-2 py-1 text-sm w-full border-none"
                  >
            ...
            <p>No pending order found for "{search}".</p>
          )
        ) : data && data.length > 0 ? (
          <div>
            {data.map((elem, index) => (
