...
const page = () => {
  ...
  const fetchOrder = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/users/getcategoryorder?category=Completed&limit=${limit}&page=${page}`
      );
      if (!response.ok) {
  ...
  const SearchHandler = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/users/searchorder?title=${search}&category=Completed`
      );
      if (!response.ok) {
  ...
  return (
    <div className="h-full w-full flex flex-col  py-10 px-14 gap-10 bg-formBG rounded-md">
      <div className="py-4 px-8 flex justify-between text-xl items-center w-full border-none bg-headerBG rounded-sm shadow-sm text-textBlack">
        <h4>Completed Order List</h4>
      </div>

      {/* Search Section */}
      <div className="flex justify-between items-center px-6 w-full border-none shadow-sm bg-headerBG rounded-sm text-textBlack">
      ...
      <div>
        {search ? (
          searchMergedOrders.length ? (
            ...
            <p>No Completed order found for "{search}".</p>
          )
        ) : data && data.length > 0 ? (
          <div>
            {data.map((elem, index) => (
