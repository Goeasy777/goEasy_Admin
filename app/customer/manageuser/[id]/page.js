"use client";
import { addWallethandeler } from "@/store/Actions/adminActions";
...
const Report = ({ params }) => {
  ...
  const fetchUser = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/users/getuserbyid/${id}`
      );
      if (!response.ok) {
  ...
  const handleAddBalance = async (e) => {
    ...
    await dispatch(addWallethandeler(formData));
    setAmount("");
    fetchUser();
    fetchWallet();
  };
  ...
  const fetchWallet = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/users/getwallet?id=${id}&page=${page}&limit=${limit}`
      );
      if (!response.ok) {
  ...
  return (
    <div className="flex flex-col bg-white  w-full min-h-screen text-[#5A6ACF] py-6 px-4 gap-8 rounded-md shadow-sm">
          ...
          <div className="flex items-center bg-white p-4 rounded-sm w-full md:w-1/6  ">
            ...
            <div>
              ...
              <p className="mt-1 text-base font-semibold text-black">
                ...
                {userdata?.orderId?.length || 0} Orders
              </p>
            </div>
          </div>
      ...
      <div className="bg-gray-100 flex flex-col gap-5 p-5 rounded-md">
        <div className="bg-white text-black font-semibold px-8 py-5 rounded-sm">
          <h3 className="text-xl">Username Wallet Transaction Report</h3>
        </div>
